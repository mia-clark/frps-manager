...
func spawnUpdater(u *Updater, mode Mode, targetVersion string) error {
	...
	if mode == ModeSystemd && hasExec("systemd-run") {
		...
		// aborts with "Unit frpsmgrd-selfupdate.service already exists" (--collect
		...
		_ = exec.CommandContext(stopCtx, "systemctl", "stop", "frpsmgrd-selfupdate.service").Run()
		cancel()
		_ = exec.Command("systemctl", "reset-failed", "frpsmgrd-selfupdate.service").Run()

		out, err := run("frpsmgrd-selfupdate")
		if err != nil && strings.Contains(string(out), "already exists") {
			...
			out, err = run(fmt.Sprintf("frpsmgrd-selfupdate-%d", time.Now().UnixNano()))
		}
		if err != nil {
