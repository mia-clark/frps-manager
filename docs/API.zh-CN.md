...
#### `GET /api/v1/system/update/log` — 读取自更新步骤日志

返回 `update.log` 的尾部（≤64 KiB），供网页在更新进行中**实时显示步骤**（下载/解压/安装/重启）。
每次 `POST /system/update` 启动时会重置为全新表头。重启窗口期接口短暂不可达属正常，前端轮询容忍即可。

```json
{ "content": "[*] 准备自更新: 0.0.22 -> v0.0.23\n▶ [1/5] 检测环境与当前版本\n[+] 下载完成 (代理): ... 10M\n…" }
```
